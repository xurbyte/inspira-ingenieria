...
import { ArrowLeft, Home, Hammer, TreePine, MapPin } from "lucide-react"
...
export default function ViviendasPage() {
  ...
  const projects = allProjects.viviendas
  ...
  const filteredProjects = projectsByType[selectedType]

  const handleProjectClick = (project: DatabaseProject) => {
    router.push(`/proyectos/viviendas/${project.slug}`)
  }

  const filterTabs: Array<{ key: "tradicional" | "steelframe" | "woodframe"; label: string; icon: React.ReactNode }> = [
    { key: "tradicional", label: "Tradicional", icon: <Home className="h-4 w-4" /> },
    { key: "steelframe", label: "Steel Frame", icon: <Hammer className="h-4 w-4" /> },
    { key: "woodframe", label: "Wood Frame", icon: <TreePine className="h-4 w-4" /> },
  ]
      ...
      <div className="container mx-auto px-4">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="mb-8">
            ...
            <h1 className="heading-section text-foreground mb-2">Proyectos de Viviendas</h1>
            <p className="text-muted-foreground font-medium">Soluciones residenciales con diferentes sistemas constructivos</p>
          ...
          {!loading && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              {filteredProjects.length === 0 ? (
                ...
                filteredProjects.map((project: DatabaseProject, index: number) => (
                  <div
                    key={project.id}
